...
def main():
    ...
    ap.add_argument("--out", default=r".\tools\PROJECT_TREE.txt", help="出力ファイル（既定: <root>/PROJECT_TREE.txt）")
    ...
    text = render_tree_text(root, max_depth, matcher, include_files=not args.dirs-only if False else not args.dirs_only)  # keep arg name compatibility
